...
**스크립트 한 번 실행**으로 끝내고, 스크립트로 어려운 부분(로그인·화면 설정)은 **사진과 함께**
따라갑니다.
...
├── scripts/
│   ├── install-windows.ps1         Windows 일괄 설치 (winget)
│   ├── check-windows.ps1           Windows 설치 확인
│   ├── install-macos.sh            macOS 일괄 설치 (Homebrew)
│   └── check-macos.sh              macOS 설치 확인
...
2. 🪟 Windows 는 `docs/02`, 🍎 macOS 는 `docs/03` 를 따라 **설치 스크립트 실행**
3. ✅ `docs/04` 의 **확인 스크립트**로 전부 설치됐는지 검증
4. 🔐 수동 단계: **Codex 로그인**, **Git 이름/이메일 설정**, VS Code 첫 실행
5. 🧪 `exercises/01` 체크리스트 채우기 → 완료!
