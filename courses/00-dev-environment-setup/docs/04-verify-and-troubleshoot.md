...
> 확인 스크립트(`check-windows.ps1` / `check-macos.sh`)는 도구마다 **버전을 출력해 ✅/❌** 를 표시합니다.
> ❌ 가 나오면 아래 표에서 해당 도구를 찾아 조치한 뒤 **새 터미널에서** 다시 실행하세요.
...
확인하세요. Windows 는 특히 **PowerShell 7(pwsh)** 로 새로 여세요.
...
| 스크립트 실행이 차단됨 (`...실행할 수 없습니다`) | `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` 를 먼저 실행했는지 확인 |
