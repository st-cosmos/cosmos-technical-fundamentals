...
1. **터미널이란?** — 터미널 · 셸 · OS 가 각각 하는 일, 명령의 생김새
2. **폴더 오가기** — `pwd` · `ls` · `cd` · `mkdir` (서랍장 비유)
3. ★ **경로** — 절대 경로 vs 상대 경로, `.` `..` `~`
4. **내 IP 확인** — `ipconfig` / `ifconfig`

> 핵심 한 줄: **터미널에서 치는 명령 = 셸이 프로그램을 실행시켜 OS 에 일을 시키는 것.** 탐색기가 마우스로 하는 일과 같습니다.
...
<div class="lead-sub">터미널 · 셸 · 운영체제 — 누가 무엇을 하나</div>
...
| 층 | 정체 | 하는 일 | 예 |
|----|------|---------|----|
| **터미널** | 글자 입출력 **창** (옛날엔 진짜 단말기, 지금은 에뮬레이터) | 키 입력을 셸에 전달, 셸의 출력을 화면에 | Windows Terminal, Terminal.app, VS Code 터미널 |
| **셸** | 명령을 **해석·실행하는 프로그램** | 한 줄을 읽어 → 명령·인자로 쪼개고 → 그 이름의 **프로그램을 찾아 실행** → 결과를 돌려줌 | PowerShell 7, zsh, bash |
| **OS(커널)** | 자원 관리자 | 프로그램의 요청(파일 열기·폴더 만들기·소켓 열기)을 **시스템 콜**로 받아 하드웨어에 수행 | Windows, macOS |

> `mkdir`·`git`·`uv` 는 **셸이 실행해 주는 별개 프로그램**. 탐색기(GUI)도 같은 OS 기능을 마우스로 부를 뿐 — **셸 = OS 와 대화하는 글자 인터페이스.**
...
셸은 한 줄을 **띄어쓰기**로 쪼개 첫 단어를 프로그램으로, 나머지를 그 프로그램에 넘길 값으로 봅니다.
...
- **프로그램** 무엇을 — 첫 단어. 셸이 PATH 에 등록된 폴더에서 찾아 실행 · **하위명령** 큰 프로그램 안의 기능 이름
- **인자(argument)** 무엇에 대해 — 파일·폴더·이름. 공백이 있으면 `"My Folder"`
- **옵션(option/flag)** 어떻게 — `-r` 짧은 형(붙여 쓰기 가능 `-la`) / `--recursive` 긴 형(읽기 쉬움). 값이 따르기도: `-m "..."`, `--port 8000`

> 모르는 명령은 **`명령 --help`** (PowerShell 자체 명령은 `Get-Help 명령`) — 옵션 목록이 다 나오니 외울 필요 없음
...
- **터미널**(글자 창) 안에서 **셸**(PowerShell 7 / zsh)이 한 줄을 읽어 **프로그램을 실행**하고, 프로그램은 **OS** 에 파일·네트워크 작업을 요청
- 명령 = **프로그램 + 인자 + 옵션**(`-r` / `--recursive`) — 모르면 `--help`
