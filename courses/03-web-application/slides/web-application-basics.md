...
<div class="eyebrow">COSMOS · 기술 기초 교육 03</div>
...
> (05·06 강좌에서 이 화면을 서버·ESP32 와 연결)
...
> "서버" 는 거창한 게 아니라 **요청을 받아 응답하는 프로그램**. 05 강좌에서 노트북에 직접 띄움.
...
> 이걸 **서버에 저장해 다 같이 보게**(+ ESP32 LED) 만드는 것이 05·06 강좌. (`exercises/01`)
...
**개발자 도구 (F12)**: Elements(구조·CSS) · Console(오류·JS) · Network(요청/응답 — 05 에서 핵심)
...
<div class="small">다음: 04-python-programming (서버를 만들 파이썬) → 05-web-server-python (이 화면 + FastAPI 서버 + WebSocket)</div>
