...
> 05 강좌의 서버 상태 `{"on": True, "by": "철수"}` 가 바로 딕셔너리입니다.
...
이 구조가 실습 02(LED 이력 기록기)와 05 강좌 서버의 핵심 데이터입니다.
...
datetime.now().strftime("%H:%M:%S")     # "14:03:05"  ← 05 서버가 time 을 만드는 방식
...
이 30줄이 실습 02 의 뼈대이고, 여기에 `while True` 입력 루프를 붙이면 LED 이력 기록기가 됩니다. 05 강좌에서는 같은
...
그다음 [05-web-server-python](../../05-web-server-python/README.md) 으로.
