...
> 이 기초 위에 06 강좌의 **FastAPI 서버**가 올라갑니다. 딕셔너리 = 서버 상태 = JSON.
...
led = {"on": False, "by": "아직 아무도", "time": "-"}     # ← 06 서버의 상태 그대로
...
datetime.now().strftime("%H:%M:%S")        # 06 서버가 time 을 만드는 방식
```
...
> `led` 딕셔너리와 `set_led()` 가 06 강좌에서 **서버 상태와 `PUT /api/led`** 로 그대로 바뀝니다.
...
- 실습 02 의 딕셔너리·함수가 06 의 서버가 된다

<div class="small">docs/ 코드는 <code>uv run python</code> 셸에 직접 쳐 보며 읽기 · 다음: 06-web-server-python</div>
