...
> 04 강좌의 `led` 딕셔너리와 `set_led()` 함수가 **그대로 API 가 되는** 것을 보게 됩니다. 테스트는 `/docs` 에서.
...
04 강좌 LED 이력 기록기의 데이터를 그대로 가져옵니다.
...
> 데이터베이스에 저장합니다. (04 강좌의 `json.dump` 를 붙이면 끝 — 더 해보기)
...
- 04 강좌 `set_led(on, by)` 함수와 비교해 보세요 — 인자가 `cmd` 하나로 묶였을 뿐 하는 일이 같습니다.
...
> `422` 를 돌려줍니다. 04 강좌의 타입 힌트가 실제로 일하는 순간.
...
API 가 생겼으니, 03 강좌의 LED 제어판 JS 가 `new Date()` 로 **화면 안에서만** 기록하던 것을 **`fetch` 로 서버에 PUT**
