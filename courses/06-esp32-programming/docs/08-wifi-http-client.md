...
> 🔗 이 실습의 **LED 서버**는 [05-web-server-python](../../05-web-server-python/README.md) 강좌에서 직접 만든 것입니다.
> 완성 예제를 `uv run` 으로 띄우고(아래 6절), 여기서는 그 서버에 붙는 **ESP32 쪽 코드**에 집중합니다.
> 05 에서 본 `GET /api/led` 와 `{on, by, time}` 규칙이 그대로 나옵니다.
...
서버는 새로 만들지 않습니다. **05-web-server-python 에서 만든 LED 서버의 완성 예제**를 그대로 실행합니다. (첫 실행은 uv 가
의존성을 내려받느라 수십 초 걸립니다. 05 실습에서 직접 만든 `led-server` 를 `--host 0.0.0.0` 으로 띄워도 됩니다.)
...
| 브라우저에 `http://0.0.0.0:8000` 을 넣었더니 안 열림 | `0.0.0.0` 은 서버가 "모든 네트워크에서 받겠다" 는 설정이지 접속 주소가 아님. 내 PC 는 `localhost`, ESP32·다른 기기는 **PC 의 실제 IP** |
