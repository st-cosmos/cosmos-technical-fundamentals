...
> 🎯 목표: ESP32에 코드를 올려 **LED를 1초 간격으로 깜빡인다.** 개발 환경이 제대로
> 동작하는지 확인하는 첫 실습.
>
...
- ESP32 DevKit 보드 + 데이터 USB 케이블
- LED + 220Ω 저항 + 브레드보드 → **GPIO23 ──[220Ω]──▶|── GND** (LED 긴 다리가 저항 쪽)
  (부품이 없으면 내장 LED 로 — 코드의 `LED_PIN` 을 2 로)
...
```cpp
const int LED_PIN = 23;           // GPIO23 에 연결한 LED
...
3. `SUCCESS` 가 뜨면 완료. GPIO23 의 LED 가 **1초마다 깜빡**입니다. 🎉
...
3. **내장 LED 로 바꿔 보기**: `LED_PIN = 2` 로 바꾸면 보드의 파란/빨간 내장 LED 가 대신 깜빡입니다.
...
| 업로드는 됐는데 안 깜빡임 | EN(리셋) 버튼 한번 누르기. LED 방향(긴 다리가 저항 쪽)과 GPIO23 배선 확인 |
