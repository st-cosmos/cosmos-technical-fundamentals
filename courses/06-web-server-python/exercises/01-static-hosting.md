...
> 🎯 목표: **FastAPI 서버를 띄우고**, 04 강좌에서 만든 LED 제어판(HTML/CSS/JS)을 그 서버가 내려주게 한다.
...
- 04 강좌 `examples/01-led-panel/` 의 세 파일 (`index.html`, `style.css`, `app.js`) — 또는 직접 만든 것
...
2. 04 강좌의 `examples/01-led-panel/index.html`, `style.css`, `app.js` 를 `static/` 안으로 복사합니다.

```bash
mkdir static
# 저장소 최상위 기준 경로 예 — 본인 위치에 맞게
cp ../cosmos-technical-fundamentals/courses/04-web-application/examples/01-led-panel/*.* static/
```
...
`--reload` 덕분에 서버가 자동 재시작됩니다. **http://localhost:8000/** → LED 제어판! 켜기/끄기가 04 때와 똑같이 동작합니다.
