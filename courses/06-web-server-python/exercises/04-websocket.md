...
ESP32([03 강좌 docs/08](../../03-esp32-programming/docs/08-wifi-http-client.md))가 `GET /api/led` 를 polling 하는 것도 그대로 동작합니다.
...
2. **이력 함께 push**: 서버가 `history` 리스트(최근 10건)도 같이 보내고, 04 강좌처럼 `<ul>` 에 표시
...
04 의 화면과 05 의 파이썬이 서버로 이어졌고, 03 의 ESP32 까지 같은 서버를 봅니다.
