...
<div class="meta">
uv + FastAPI + uvicorn · 04 의 LED 제어판을 서버에<br>
대상: 04(웹 페이지)·05(파이썬) 를 마친 부원
</div>
...
> 05 의 `led` 딕셔너리와 `set_led()` 가 **그대로 서버**가 됩니다. 마지막엔 두 창이 같은 LED 를 **즉시** 공유.
> 03 강좌에서 ESP32 가 붙였던 서버가 **바로 이것** — 이제 그 코드를 직접 만듭니다.
...
## static/ 폴더 서빙 — 04 의 LED 제어판을 서버에
...
- `http://localhost:8000/` → LED 제어판. 동작은 04 와 같지만 **주소가 http://**
...
## GET / PUT `/api/led` — 05 의 딕셔너리가 그대로
...
> 05 의 `set_led(on, by)` 와 비교 — 인자가 `cmd` 로 묶였을 뿐. 타입 힌트가 **실제 검증**(틀리면 422)을 한다.
...
> 04 의 `setLed()` 가 `new Date()` 로 **화면 안**에 기록하던 것을 → **서버에 PUT** 하고 **GET 으로 받아 그리기**로.
...
- 서버 규칙 `{on, by, time}` 은 05 기록기·03 ESP32 와 동일 — **한 서버, 여러 클라이언트**

<div class="small">실습: exercises/01 호스팅 → 02 API → 03 polling ★ → 04 WebSocket ★ · 다음: 07-ai (이 서버에 AI 로 기능 추가)</div>
